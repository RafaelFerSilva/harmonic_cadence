...
cf. `i7-funk-anchor`) ou `Emp` (empréstimo modal). Além disso, o gate de trítono do
`songbook_baseline.py` **não executa** hoje (chama um método inexistente, exceção engolida).
Portanto o sistema SHALL expor a leitura trítono-real-não-dominante como um **ledger de
curadoria** (worklist a adjudicar por Chediak), **não** como gate que falha o build. O comando
`corpus gates` SHALL reportar a contagem do ledger de forma informativa, sem sair com erro por
causa dele. A adjudicação das exceções e a correção do bug do baseline SHALL ser tratadas numa
change separada.
...
#### Scenario: Ledger identifica cada ocorrência
