...
The `cifra_core` music-theory layer provides the foundational primitives for harmonic analysis: a spelling-preserving note model, interval computation, chord realization into pitch classes, and scales/modes as data. These primitives let downstream analysis reason about chord tones and tonal content rather than raw chord-symbol strings.
