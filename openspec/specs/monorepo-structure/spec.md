...
The project lives in a single repository with cleanly separated packages (`cifra_core`, `cifra_scraper`, `harmonic_analysis`), a unified packaging toolchain, and unified entry points for build, test, run, and deploy. Dependency direction is enforced so the shared core stays decoupled and the analyzer reaches cifra data only through the `SongProvider` port.
