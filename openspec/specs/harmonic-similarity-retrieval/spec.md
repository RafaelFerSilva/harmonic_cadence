# harmonic-similarity-retrieval Specification

## Purpose

Recuperação de músicas por similaridade harmônica sobre o corpus persistido. Para cada música do
run corrente constrói-se um embedding harmônico (reusando o `Fingerprint` de `style_fingerprint`),
transposição-invariante e derivado só do corpus, sem anotação externa. A similaridade de cosseno
(reusando `style_fingerprint.similarity`) materializa os top-K vizinhos numa tabela derivada
(`song_neighbor`) + view, expostos por `harmonic corpus similar --song <slug>`. O overlay é
estritamente **descritivo**: não reescreve `function_code`, não arbitra centro tonal, não altera os
3 gates duros/baseline/`detect_key`, e a similaridade nunca é veredito de qualidade ou correção.
