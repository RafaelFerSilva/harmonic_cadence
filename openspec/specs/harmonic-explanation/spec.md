# harmonic-explanation Specification

## Purpose

The system turns a harmonic analysis into a human-readable explanation through an `Explainer` port, with a deterministic offline `TemplateExplainer` default and an optional LLM explainer over a pluggable provider layer selected only by explicit configuration.
