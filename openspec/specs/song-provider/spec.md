# song-provider Specification

## Purpose

The `SongProvider` port abstracts cifra retrieval for the analyzer, decoupling its domain and service layers from any concrete transport. It defines a typed error taxonomy and ships HTTP, in-process, and caching adapters so songs can be fetched over the Flask service, in-process without a server, or served from a local cache with configurable policy.
