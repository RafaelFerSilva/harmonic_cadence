# tritone-adjudication Specification

## Purpose

Manter uma camada de **anotação (PRATA)** sobre o ledger de trítono não-dominante: um corpus
curado, tipado e em código de **vereditos** citados de Chediak para cada ocorrência
trítono-real-não-dominante do corpus. Cada veredito nasce da geometria real do acorde (raiz vs.
tônica + resolução) e carrega uma citação obrigatória (obra/volume/página) — sem página, o fato
não existe (muro de copyright = só fatos citados). O corpus **anota, nunca reescreve**: os valores
`function_code`/`degree` do coder permanecem intocados; o resíduo genuinamente indecidível é
**declarado** (`ambiguous`), não escondido; e uma auditoria anti-drift garante que crescer o corpus
ou mudar a extração nunca deixe ocorrência sem veredito nem veredito órfão.
