...
**Frentes abertas (handoff — ver ROADMAP "Sequência sugerida" #6-8):**
- **#6 Crescer os invariantes funcionais** *(maior valor)*: hoje `scripts/songbook_baseline.py`
  só checa "trítono real ⇒ dominante" (62/62). Somar diminutos-por-tipo (XXI-XXII), ii-V (XIX) e
  a taxonomia das 5 cadências (XXXII) transforma o baseline num **gate de regressão funcional**
  que protege a teoria destilada. Tudo transposição-invariante, sobre `cifras/*.md`.
- **#7 Worklist de corroboração**: as ≈17 divergências `detect_key` × `chediak_functional_center`
  são onde vale consultar o Chediak para adjudicar e evoluir a detecção — **sem** reamarrar no CC.
